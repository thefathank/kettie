  ...
  const handleSendEmail = async (lesson: any) => {
    try {
      ...
      const { data: videos } = await supabase
        .from("client_videos")
        .select("video_url")
        .in("id", lesson.exercises?.flatMap((e: any) => e.video_ids || []) || []);

      const exercisesWithVideos = lesson.exercises?.map((exercise: any) => ({
        ...exercise,
        videoUrls: exercise.video_ids
          ?.map((id: string) => videos?.find((v: any) => v.id === id)?.video_url)
          .filter(Boolean) || [],
      }));
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">{Array.isArray(template.exercises) ? template.exercises.length : 0}</span>
          </div>
    ...
    <Card key={lesson.id} className="hover:shadow-lg transition-shadow">
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">Exercises:</span>
            <span className="font-medium">{Array.isArray(lesson.exercises) ? lesson.exercises.length : 0}</span>
          </div>
          <div className="flex gap-2 pt-2">
            <Button
              size="sm"
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => handleSendEmail(lesson)}
            >
            ...
            <Tabs defaultValue="scheduled" className="w-full">
              ...
              <TabsContent value="scheduled" className="mt-6">
                {lessonsLoading ? (
                  ...
                  <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                      <p className="text-muted-foreground">No scheduled lessons</p>
                    </CardContent>
                  </Card>
                )}
              </TabsContent>
              ...
              <TabsContent value="draft" className="mt-6">
                {lessonsLoading ? (
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {draftLessons.map(renderLessonCard)}
                  </div>
                ) : (
                  <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                      <p className="text-muted-foreground">No draft lessons</p>
                    </CardContent>
                  </Card>
                )}
              </TabsContent>
