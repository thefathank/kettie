    ...
    try {
      const { error } = await supabase.functions.invoke("send-client-email", {
        body: {
          ...
          date: format(new Date(note.note_date), "MMM d, yyyy"),
        },
      });
      ...
      <Card>
        <CardContent className="pt-6 text-center py-12">
          ...
          <p className="text-muted-foreground">
            No progress notes yet. Add your first note to start tracking progress!
          </p>
        </CardContent>
      </Card>
    );
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      if (client?.email) {
                        handleSendEmail(note, client.email, client.full_name);
                      } else {
      ...
      <AlertDialog open={!!deletingNoteId} onOpenChange={(open) => !open && setDeletingNoteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deletingNoteId && handleDelete(deletingNoteId)}>
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
